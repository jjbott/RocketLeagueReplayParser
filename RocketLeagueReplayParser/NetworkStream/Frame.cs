...
namespace RocketLeagueReplayParser.NetworkStream
{
    public class Frame
    {
        ...
        public static Frame Deserialize(int maxChannels, ref List<ActorState> existingActorStates, string[] objectIdToName, IDictionary<string, ClassNetCache> classNetCacheByName, int replayVersion, BitReader br)
        {
            ...
            while (br.ReadBit())
            {
                lastActorState = ActorState.Deserialize(maxChannels, existingActorStates, f.ActorStates, objectIdToName, classNetCacheByName, replayVersion, br);
        ...
        public void Serialize(int maxChannels, ref Dictionary<UInt32, ActorState> newActorsById, int replayVersion, BitWriter bw)
        {
            bw.Write(Time);
            ...
            foreach (var deletedActor in ActorStates.Where(a => a.State == ActorStateState.Deleted))
            {
                ...
                deletedActor.Serialize(maxChannels, null, replayVersion, bw);
            }
            ...
            foreach (var newActor in ActorStates.Where(a => a.State == ActorStateState.New))
            {
                ...
                newActor.Serialize(maxChannels, null, replayVersion, bw);
            }
            ...
            foreach (var existingActor in ActorStates.Where(a => a.State == ActorStateState.Existing))
            {
                ...
                existingActor.Serialize(maxChannels, newActorsById, replayVersion, bw);
            }
